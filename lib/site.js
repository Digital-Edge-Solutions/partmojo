...
export const OPERATOR = "Edge Digital Solutions LTD";
export const COMPANY_NUMBER = "11540507";
export const COMPANY_URL = "https://find-and-update.company-information.service.gov.uk/company/11540507";
// partmojo.com has no MX records (can't receive mail), so the public contact is the business inbox.
export const CONTACT_EMAIL = "hello@digitaledge.uk";
export const TAGLINE = "Never buy the wrong part again.";
export const PRODUCT_IMAGE = `${BASE}/filter.svg`;

// Owned, per-page branded spec-card image (served by /api/og). Unique per page,
// no licensing risk; doubles as the social/OpenGraph preview image.
export function ogImage({ code, brand, kind, specs } = {}) {
  const p = new URLSearchParams();
  p.set("code", code || BRAND);
  if (brand) p.set("brand", brand);
  if (kind) p.set("kind", kind);
  if (specs && specs.length) p.set("specs", specs.filter(Boolean).join(" | "));
