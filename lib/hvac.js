...
// residential systems. ----
export const SIZES = [
  { w: 10, h: 20 }, { w: 12, h: 12 }, { w: 12, h: 20 }, { w: 12, h: 24 }, { w: 12, h: 30 }, { w: 12, h: 36 },
  { w: 14, h: 14 }, { w: 14, h: 18 }, { w: 14, h: 20 }, { w: 14, h: 24 }, { w: 14, h: 25 }, { w: 14, h: 30 },
  { w: 15, h: 20 }, { w: 16, h: 16 }, { w: 16, h: 20 }, { w: 16, h: 24 }, { w: 16, h: 25 }, { w: 16, h: 30 },
  { w: 18, h: 18 }, { w: 18, h: 20 }, { w: 18, h: 24 }, { w: 18, h: 25 }, { w: 18, h: 30 },
  { w: 20, h: 20 }, { w: 20, h: 24 }, { w: 20, h: 25 }, { w: 20, h: 30 }, { w: 20, h: 36 },
  { w: 22, h: 22 }, { w: 24, h: 24 }, { w: 24, h: 30 }, { w: 25, h: 25 }, { w: 30, h: 30 }, { w: 30, h: 36 },
];
...
// Aftermarket brands buyers actually search for (all on Amazon Associates).
export const HVAC_BRAND_DATA = [
  { name: "Filtrete", rating: "MPR", blurb: "3M's Filtrete uses its own MPR scale (300–2800). MPR 600≈MERV 8, MPR 1000–1200≈MERV 11, MPR 1500–1900≈MERV 13.", merv: [8, 11, 13] },
  { name: "Honeywell", rating: "MERV/FPR", blurb: "Sold widely at Home Depot under both MERV and the store's FPR scale. Reliable mid-range pleated filters.", merv: [8, 11, 13] },
  { name: "Nordic Pure", rating: "MERV", blurb: "Value MERV-rated pleated filters in a huge range of sizes, including hard-to-find and custom cuts.", merv: [8, 11, 13] },
  { name: "Aerostar", rating: "MERV", blurb: "Budget-friendly MERV-rated pleated filters, frequently sold in multi-packs — good cost-per-filter.", merv: [8, 11, 13] },
  { name: "Filterbuy", rating: "MERV", blurb: "US-made pleated filters across MERV 8/11/13 and every common size, with subscription options.", merv: [8, 11, 13] },
  { name: "FilterKing", rating: "MERV", blurb: "Made-to-order pleated filters and subscriptions; strong for non-standard sizes.", merv: [8, 11, 13] },
];
export const HVAC_BRANDS = HVAC_BRAND_DATA.map((b) => b.name);
export const hvacBrandBySlug = Object.fromEntries(
  HVAC_BRAND_DATA.map((b) => [slugify(b.name), { ...b, slug: slugify(b.name) }])
);

// MERV-vs-MERV comparison pages, per size (e.g. "16x25x1 MERV 8 vs MERV 11").
// Depth fixed at 1" (the everyday grille filter people actually compare).
const MERV_PAIR_DEFS = [
  [8, 11], [11, 13], [8, 13],
];
export const HVAC_MERV_COMPARES = [];
for (const s of SIZES) {
  for (const [a, b] of MERV_PAIR_DEFS) {
    HVAC_MERV_COMPARES.push({
      w: s.w, h: s.h, depth: 1, a, b,
      sizeSlug: `${s.w}x${s.h}`,
      dims: `${s.w}x${s.h}x1`,
      slug: `${s.w}x${s.h}x1-merv-${a}-vs-${b}`,
    });
  }
}
export const hvacCompareBySlug = Object.fromEntries(HVAC_MERV_COMPARES.map((c) => [c.slug, c]));
